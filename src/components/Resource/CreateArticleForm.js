...
const CreateArticleForm = ({ addNewArticle }) => {
  ...
  const handleFileUpload = async (file, index) => {
    ...
    //     formData
    //   );
    //   const newBlocks = [...contentBlocks, { type: 'image', value: response.data.secure_url }];
    //   // newBlocks[index].value = response.data.secure_url;
    //   setContentBlocks(newBlocks);
    // } catch (error) {
    //   console.error('Error uploading file:', error);
    // }
    const newBlocks = [...contentBlocks, { type: 'image', value: 'https://www.befunky.com/images/prismic/5ddfea42-7377-4bef-9ac4-f3bd407d52ab_landing-photo-to-cartoon-img5.jpeg?auto=avif,webp&format=jpg&width=863' }];
    // newBlocks[index].value = response.data.secure_url;
    setContentBlocks(newBlocks);
    setIsDisable(false);
  };
